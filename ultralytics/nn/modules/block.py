...
class LDCM(nn.Module):
    """Lightweight Direction-Context Module for directional and contextual feature enhancement."""

    def __init__(self, c1: int, c2: int | None = None, e: float = 0.5):
        """Initialize the LDCM module.
        ...
        c2 = c1 if c2 is None else c2
        cm = max(int(c1 * e), 1)
        self.cv1 = Conv(c1, cm, 1, 1)
        self.local = DWConv(cm, cm, 3)
        self.dir_h = DWConv(cm, cm, (1, 7))
        self.dir_v = DWConv(cm, cm, (7, 1))
        self.dir_fuse = Conv(cm, cm, 1, 1)
        self.context = nn.Sequential(nn.AdaptiveAvgPool2d(1), nn.Conv2d(cm, cm, 1), nn.Sigmoid())
